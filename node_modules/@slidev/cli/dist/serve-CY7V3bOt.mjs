import { a as promptForOptionalInstallation, c as resolveImportUrl, i as isInstalledGlobally, l as resolveSourceFiles, n as getRoots, o as resolveEntry, r as importOptionalDependency, s as resolveImportPath, t as createResolver, u as toAtFS } from "./resolver-BIhhHpe7.mjs";
import { builtinModules } from "node:module";
import process from "node:process";
import path, { basename, dirname, isAbsolute, join, relative, resolve, win32 } from "pathe";
	...
	return {
		...
		async transform(code, id) {
			...
			await Promise.all(extractImportSources(code, id).map(async ({ value, start }) => {
				if (isPublicAsset(value, config)) return;
...
*/
function isPublicAsset(value, config) {
	if (!config.publicDir || !value.startsWith("/") || value.startsWith("/@")) return false;
	const publicDir = normalizeFsPath(config.publicDir);
	const publicPath = normalizeFsPath(path.join(publicDir, cleanUrl$1(value).slice(1)));
...
async function resolveOptions(entryOptions, mode) {
	...
	let themeRaw = entryOptions.theme || initialLoaded.headmatter.theme;
...
function generateIdForToken(token) {
	...
	else return `task-item-${Math.ceil(Math.random() * 1e7 - 1e3)}`;
}
...
function resolveSnippetImport(lineText, userRoot, slide, allowedRoots = [userRoot]) {
	...
	meta = meta.trim();
	if (!isPathInsideRoots(src, allowedRoots)) throw new Error(`Code snippet path escapes the project root: ${src}`);
...
function MarkdownItSnippet(md, { userRoot, userWorkspaceRoot, roots, data: { watchFiles, slides } }) {
...
const frontmatterSchema = z.record(z.string(), z.any()).optional().describe("Slide frontmatter (YAML headmatter of the slide) as an object, e.g. { \"layout\": \"two-cols\" }");
/**
...
*/
function isSafeDrawingKey(dir, key) {
...
async function createPWAPlugin(options) {
	const matchMode = (mode) => mode === true || mode === options.mode;
	if (!matchMode(options.data.config.pwa)) return createPWARegisterStubPlugin();
	...
	return VitePWA({
		...
		manifest: {
			name: options.data.config.title || "Slidev",
			short_name: "Slidev",
			start_url: base,
			display: "fullscreen",
			theme_color: "#121212",
...
* is missing.
*/
async function resolveVitePWA() {
	let mod = await importOptionalDependency(PWA_PACKAGE);
	...
	if (!VitePWA) throw new Error(`[slidev] Failed to load "${PWA_PACKAGE}", which is required by the "pwa" option.`);
	return VitePWA;
}
/**
...
*/
function createPWARegisterStubPlugin() {
	...
	return {
		...
		load(id) {
			if (id === RESOLVED_ID) return "export function registerSW() { return () => Promise.resolve() }";
		}
	};
}
//#endregion
