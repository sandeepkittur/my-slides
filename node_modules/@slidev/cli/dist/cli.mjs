import { i as isInstalledGlobally, o as resolveEntry } from "./resolver-BIhhHpe7.mjs";
import { c as getThemeMeta, d as loadSetups, f as updateFrontmatterPatch, l as resolveTheme, o as resolveOptions, s as parser, t as createServer, u as version } from "./serve-CY7V3bOt.mjs";
...
cli.command("build [entry..]", "Build hostable SPA", (args) => exportOptions(commonOptions(args)).option("out", {
	...
	const { build } = await import("./build-rtQk8gSn.mjs");
...
cli.command("mcp [entry]", "Start an MCP (Model Context Protocol) server over stdio for AI agents to inspect and edit the slides", (args) => commonOptions(args).strict().help(), async ({ entry }) => {
	const { startMcpStdioServer } = await import("./stdio-TyXSpxhg.mjs");
...
cli.command("export [entry..]", "Export slides to PDF", (args) => exportOptions(commonOptions(args)).strict().help(), async (args) => {
	...
	const { exportSlides, getExportOptions } = await import("./export-B_maVZXE.mjs");
...
cli.command("export-notes [entry..]", "Export slide notes to PDF", (args) => args.positional("entry", {
	...
	const { exportNotes } = await import("./export-B_maVZXE.mjs");
	const candidatePort = await getPort(12445);
	for (const entryFile of entry) {
...
function printInfo(options, port, base, remote, tunnelUrl, publicIp) {
	...
	if (port) {
		...
		if (options.mode === "dev" && options.data.config.mcp !== false) console.log(`${dim("  mcp server    ")}     > ${blue(`http://localhost:${bold(port)}/__mcp`)}`);
		if (options.inspect) console.log(`${dim("  vite inspector")}      > ${yellow(`${baseUrl}/__inspect/`)}`);
		let lastRemoteUrl = "";
		if (remote !== void 0) {
			Object.values(os.networkInterfaces()).forEach((v) => (v || []).filter((details) => String(details.family).endsWith("4") && !details.address.includes("127.0.0.1")).forEach(({ address }) => {
