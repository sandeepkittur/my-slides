...
function hijackPlugin(plugin, ctx) {
	...
	hijackHook(plugin, "transform", async (fn, context, args, order) => {
		...
		const end = Date.now();
		const result = error ? "[Error]" : typeof _result === "string" ? _result : _result === null || _result === void 0 || (_result$code = _result.code) === null || _result$code === void 0 ? void 0 : _result$code.toString();
		if (ctx.filter(id)) {
